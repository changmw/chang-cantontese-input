...
_screen.AddProperty("StartUpFolder", fullpath(curdir()))
_screen.AddProperty("Quit", .f.)
_screen.AddProperty("MutexHandle", -1)
_screen.AddProperty("old_winstate", _screen.WindowState)
_screen.AddProperty("old_top", _screen.top)
_screen.AddProperty("min_height", 300)
_screen.AddProperty("min_width", 450)
_screen.AddProperty("FormInit", .f.)

_screen.ShowTips=.t.
_screen.Enabled=.f.

if _vfp.StartMode=4
	_screen.top=-2*sysmetric(2)
endif
...
Define class MyPage3 as Page
	caption="\<Update"
	Add Object text2 as MyUnicodeTextbox with ;
		top=15, left=5, ;
		Height=30, width=100
	Add Object cmdSave as MyCommandButton with ;
		top=50, left=5

	procedure cmdSave.click()
		m.haha=strconv(This.parent.text2.Text,12)
		CopyUnicodeText2Clipboard(m.haha)
	endproc
EndDefine

Define class MyPage4 as Page
	caption="\<About"
	WasHit=.f.

	Add Object lblBig5 As Label With ;
		top=140, Left=15, FontSize=10, FontItalic=.t., FontBold=.t., ;
		AutoSize=.t., ForeColor=rgb(255,64,64), ;
		Caption="NOTE: This program uses Big5 charset NOT Unicode!"
	Add Object lblFreeware As Label With ;
		top=175, Left=15, FontSize=10, AutoSize=.t., ;
		Caption="Freeware (2008-2014)", ForeColor=rgb(128,64,0)
	Add Object cmdWebsite as MyCommandButton with ;
		top=160, Left=170, Width=240, FontSize=8, FontItalic=.t., ForeColor=rgb(128,64,0), ;
		Caption="http://sites.google.com/site/changmw"
	Add Object lblEmail As Label With ;
		top=200, Left=15, FontSize=10, AutoSize=.t., ForeColor=rgb(128,64,0), ;
		Caption="Programmed by Man-wai Chang"
	Add Object cmdEmail as MyCommandButton with ;
		top=190, Left=220, Width=190, FontSize=8, FontItalic=.t., ForeColor=rgb(128,64,0), ;
		Caption="Email: toylet.toylet@gmail.com"
	Add Object lblCopyRight As Label With ;
		top=225, Left=15, FontSize=10, AutoSize=.t., ForeColor=rgb(128,64,0), ;
		Caption="Powered by Micro$oft Visual Foxpro 6 (SP5)"
...
Define class MyPage1 as Page
	...
	Add Object list1 As ListBox With ;
		top=this.my_top+60,Left=this.my_left+5,Width=130,Height=177, ;
	...
	Add Object edit1 As MyUnicodeTextBox With ;
		top=this.my_top+60,Left=this.my_left+150,Width=260,Height=130, ;
		TabStop=.F., ;
		ToolTipText="Enter a maths. equation here"

	Add Object cmdClip As MyCommandButton with ;
		top=this.my_top+202,Left=this.my_left+150, Width=75, Caption="\<Copy", enabled=.t.
	Add Object cmdHistory As MyCommandButton with ;
		top=this.my_top+202,Left=this.my_left+150+75+10, Width=75,Caption="\<History", enabled=.f.
	Add Object cmdReset As MyCommandButton With ;
		top=this.my_top+202,Left=this.my_left+150+75+75+20, Width=75,Caption="\<Reset", enabled=.t.
...
*!*	form1.command1.GotFocus()
	...
	if this.parent.edit1.text=DEFAULT_CLIPTEXT
				...
				If matched.leaf
					* leaf node, no need to drill further, user chose the chinese character
					...
					if thisform.PageFrame1.Page2.chkDirect.value
						pastebig5(matched.big5)
					endif
					** unicode
					.edit1.text=.edit1.text+strconv(alltrim(matched.big5),12)
					.cntLabels.lblChangjei.Caption=matched.big5+":"+matched.changjei
					.cmdReset.enabled=.t.
					.cmdClip.enabled=.t.
					.text1.SetFocus()
				Else
	...
	* copy to windows clipboard
	with this.parent
		if !empty(.edit1.text)
			m.haha=strconv(This.parent.edit1.Text,12)
			CopyUnicodeText2Clipboard(m.haha)
			* _cliptext=.edit1.text
...
Define class MyPageFrame as PageFrame 
    ...
    top=10
    Left=10
    width=_screen.min_width-20
    height=_screen.min_height-20
					...
					* got the previous window in z-order
					...
					cBuffer = Chr(GUITHREADINFO_SIZE) + Repli(Chr(0), GUITHREADINFO_SIZE-1)
	 				IF GetGUIThreadInfo(lnThreadId, @cBuffer) = 0
	 				   messagebox("GetGUIThreadInfo() Error: "+GetLastError())
					ELSE
						...
						do case					
							case m.haha>0
								...
								? "using postmessage()"
								#endif
								PostMessage(m.haha, WM_CHAR, asc(left(m.big5,1)), 0)
								PostMessage(m.haha, WM_CHAR, asc(right(m.big5,1)), 0)
							case .f.
								* PASTE using clipboard
								...
								if .t.
									...
									* send big5 by keybd_event()
									...
									#endif
									sendkey(asc(left(m.big5,1)), 0)
									sendkey(asc(left(m.big5,1)), KEYEVENTF_KEYUP)
									sendkey(asc(right(m.big5,1)), 0)
									sendkey(asc(right(m.big5,1)), KEYEVENTF_KEYUP)
								endif
							otherwise
								#ifdef debug_caret
