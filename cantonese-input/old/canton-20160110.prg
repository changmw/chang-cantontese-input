...
#define hahaha "CANTON.DBF failed!"
...
with _screen
	...
	.AddProperty("min_height", 270)
	.AddProperty("min_width", 460)
	...
	* Add Object edit1 As MyUnicodeEdit
	...
	Procedure text01.valid()
		...
		if lastkey()=K_ESC
			return .t.
		ENDIF
		IF INLIST(LASTKEY(),19,45)
			* ALT+R (hit cmdReset), ALT+X (hit cmdClear)
			RETURN .t.
		ENDIF
		m.loObj=sys(1270)
		if type("loObj")="O"
	...
	with this.parent
		IF m.dbferr
			.CntLabels.lblCalculate.caption=hahaha
		ELSE
