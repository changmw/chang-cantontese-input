...
_screen.AddProperty("min_height", 280)
_screen.AddProperty("min_width", 440)
_screen.AddProperty("FormInit", .f.)
_screen.AddProperty("cantondbf",fullpath(curdir())+"data\canton.dbf")
	...
	procedure cmdSearch.click()
		...
		if this.caption="\<Cancel"
			...
			with this.parent
				...
				if found()
					.text22.text=strconv(alltrim(canton.big5),9)
		...
		with this.parent
			do case
				...
				case empty(.text11.text)
					if left(version(4),2)="06"
						.text11.text=FromBase64(.text12.value)
					else
					...
					* leaf node, no need to drill further, user chose the chinese character
					thisform.addone(matched.big5)
 					if left(version(4),2)=="06"
 						...
 						* more work
 						* m.xx=createbinary(fromBase64(alltrim(matched.uncd)))
					else
						** working **
						* .edit1.text=.edit1.text+strconv(alltrim(matched.big5),9)
						m.xx=strconv(alltrim(matched.uncd),14)
					endif
					thisform.PageFrame1.Page3.text21.text=m.xx
